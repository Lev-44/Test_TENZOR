...
file_formatter = logging.Formatter('%(asctime)'
                                   's - %(name)s - '
...
# Определяем фикстуру для браузера с областью действия "module"
...
class TestSbisContactsPage:
    ...
    def test_pic_sizes(self, browser):
        ...
        assert current_url == 'https://tensor.ru/about', \
            f'Ожидался URL: https://tensor.ru/abou, но был: {current_url}'
        ...
        assert (pic1_width == pic2_width ==
                pic3_width == pic4_width), 'ширина фото разная'
        assert (pic1_height == pic2_height ==
                pic3_height == pic4_height), 'высота фото разная'
